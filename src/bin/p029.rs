extern crate ooga;
use ooga::sha1::{sha1, sha1_attack};
...
fn main() {
	...
	let generate_mac = move |input: &[u8]| -> [u32; 5] {
	...
	let expected_hash = sha1(forged_msg);
	...
	while !semi_forged {
		...
		let extended_hash = sha1_attack(addition.to_vec(), hash, padded_msg_len);
...
fn padded_msg(msg: &[u8], byte_len: usize) -> Vec<u8> {
	...
	let len64b = unsafe { mem::transmute::<u64, [u8; 8]>( (l as u64).to_be() ) };
