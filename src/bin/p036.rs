	...
	pub fn gen_key(&mut self) {
		match self.salt.as_ref() {
			Some(salt) => {
				...
				// base = (B - k * g ** x)
				let mut bnctx = BigNumContext::new().unwrap();
				let mut g_x_mN = BigNum::new().unwrap();
				g_x_mN.mod_exp(&self.g, &x, &self.nist_prime, &mut bnctx).unwrap();
				let base = self.b.as_ref().unwrap().sub(&g_x_mN.mul(&self.k));

				// exp = a + u * x
				let exp = x.mul(self.u.as_ref().unwrap()).add(self.little_a.as_ref().unwrap());

				let mut s = BigNum::new().unwrap();
				s.mod_exp(&base, &exp, &self.nist_prime, &mut bnctx).unwrap();
				let k = {
					let mut hasher = Sha256::new();
					hasher.update(&s.to_vec());
					let hash = hasher.finish();
